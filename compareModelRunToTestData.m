...
matchidx = modeloffsets >= testset.LowerBound & modeloffsets <= testset.UpperBound;
...
for i = 1:testsetsize
    ...
    if (modeloffsets(i) >= testset.LowerBound(i) && modeloffsets(i) <= testset.UpperBound(i))
        result = 'MATCH';
    else
        result = 'MISMATCH';
    end
    ...
    name = sprintf('%s_AMv%s m%d vs Labels - Ex %d (ID %d Date %s%s) Offset %2d vs %2d - %2d %s', study, version, modelidx, thisinter, scid, ...
    ...
    for m = 1:nmeasures
        ...
        [xl, yl] = plotLatentCurve(ax, max_offset, align_wind, offset, (meancurvemean(:, m) + normmean(thisinter, m)), xl, yl, 'red', ':', 1.0);
        [xl, yl] = plotLatentCurve(ax, max_offset, align_wind, offset, smooth(meancurvemean(:, m) + normmean(thisinter, m),5), xl, yl, 'red', '-', 1.0);
        [xl, yl] = plotExStart(ax, ex_start, offset, xl, yl,  'black', '-', 0.5);            
        [xl, yl] = plotVerticalLine(ax, 0, xl, yl, 'cyan', '-', 0.5); % plot treatment start
        hold on;
        fill(ax, [ (ex_start + testset.LowerBound(i)) (ex_start + testset.UpperBound(i))    ...
                      (ex_start + testset.UpperBound(i)) (ex_start + testset.LowerBound(i)) ], ...
