function runAlignmentModelEMFcnFEV1Split(amRunParameters)
...
fev1max = sortrows(fev1max, {'Max'}, 'ascend');
fev1max.NTile(:) = 0;
for i = 1:npmpatients
...
ninterventionsKeep = ninterventions;

for t = 1:ntiles
    
    fprintf('Running for FEV1 split %d\n', t);
    amInterventions = amInterventionsKeep(ismember(amInterventionsKeep.SmartCareID, fev1max.ID(fev1max.NTile == t)),:);
