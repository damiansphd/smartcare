...
for i = 0:max_offset-1
    idx = find(abs(amInterventions1.Offset - amInterventions2.Offset) == i);
    if (size(idx,1) > 0)
        ...
        if i < mismatch_threshold
            ...
            for a = 1:size(idx,1)
                ...
                fprintf('%2d: Offset %2d vs %2d, Data Completeness = %.2f%% %.2f%%\n', idx(a), offset_array(idx(a),1), ...
                    offset_array(idx(a),2), datacompleteness1, datacompleteness2);
                scid = amInterventions1.SmartCareID(idx(a));
                name = sprintf('%s Prediction Comparison %sm%d vs %sm%d - Ex %d (ID %d Date %s) Offset %2d vs %2d', ...
                ...
                for m = 1:nmeasures
                    ...
                    if sigmamethod1 == 4
                        adjmeancurvemean1 = (meancurvemean1(:,m) * normstd1(idx(a), m)) + normmean1(idx(a), m);
                    else
                        adjmeancurvemean1 =  meancurvemean1(:,m) + normmean1(idx(a), m);
                    end
                    if sigmamethod2 == 4
                        adjmeancurvemean2 = (meancurvemean2(:,m) * normstd2(idx(a), m)) + normmean2(idx(a), m);
                    else
                        adjmeancurvemean2 =  meancurvemean2(:,m) + normmean2(idx(a), m);
                    end
                    ...
                    [xl, yl] = plotExStart(ax, ex_start1, amInterventions1.Offset(idx(a)), xl, yl,  'red', '-', 0.5);
                    [xl, yl] = plotExStart(ax, ex_start2, amInterventions2.Offset(idx(a)), xl, yl, 'magenta', '-', 0.5);
                    ...
                    set(gca,'fontsize',6);
                    if measures1.Mask(m) == 1 && measures2.Mask(m) == 1
                        title(sprintf('%s %.1f %.1f', measures1.DisplayName{m}, hstg1(m, idx(a), amInterventions1.Offset(idx(a)) + 1), ...
                            hstg2(m, idx(a), amInterventions2.Offset(idx(a)) + 1)), 'BackgroundColor', 'yellow');
                    elseif measures1.Mask(m) == 1
                        title(sprintf('%s %.1f %.1f', measures1.DisplayName{m}, hstg1(m, idx(a), amInterventions1.Offset(idx(a)) + 1), ...
                            hstg2(m, idx(a), amInterventions2.Offset(idx(a)) + 1)), 'BackgroundColor', 'cyan');
                    elseif measures2.Mask(m) == 1
                        title(sprintf('%s %.1f %.1f', measures1.DisplayName{m}, hstg1(m, idx(a), amInterventions1.Offset(idx(a)) + 1), ...
                            hstg2(m, idx(a), amInterventions2.Offset(idx(a)) + 1)), 'BackgroundColor', 'green');
                    else
                        title(sprintf('%s %.1f %.1f', measures1.DisplayName{m}, hstg1(m, idx(a), amInterventions1.Offset(idx(a)) + 1), ...
                            hstg2(m, idx(a), amInterventions2.Offset(idx(a)) + 1)));
                    end
                ...
                set(gca,'fontsize',6);
                title(sprintf('Overall %.1f %.1f', overall_hist1(idx(a), amInterventions1.Offset(idx(a)) + 1), ...
                    overall_hist2(idx(a), amInterventions2.Offset(idx(a)) + 1)), 'BackgroundColor', 'yellow');
