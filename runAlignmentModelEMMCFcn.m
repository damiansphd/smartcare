...
baseplotname = sprintf('%s%s_sig%d_mu%d_ca%d_sm%d_rm%d_im%d_cm%d_mm%d_mo%d_dw%d_nl%d_rs%d_ds%d', study, mversion, sigmamethod, mumethod, curveaveragingmethod, ...
    smoothingmethod, runmode, imputationmode, confidencemode, measuresmask, max_offset, align_wind, nlatentcurves, randomseed, datasmoothmethod);
detaillog = true;
...
[meancurvesumsq, meancurvesum, meancurvecount, meancurvemean, meancurvestd, amInterventions, initial_offsets, initial_latentcurve, ...
    ...
    detaillog, sigmamethod, smoothingmethod, runmode, randomseed, fnmodelrun);
fprintf('%s - ErrFcn = %.8f\n', run_type, qual);
...
save(fullfile(basedir, subfolder, outputfilename), 'amDatacube', 'amIntrDatacube', 'amIntrNormcube', ...
    ...
    'sigmamethod', 'mumethod', 'curveaveragingmethod', 'smoothingmethod', 'datasmoothmethod', ...
    'measuresmask', 'runmode', 'randomseed', 'imputationmode', 'confidencemode', 'printpredictions', ...
