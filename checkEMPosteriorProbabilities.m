...
[temp_meancurvedata, temp_meancurvesum, temp_meancurvecount, temp_meancurvemean, temp_meancurvestd] = amEMRemoveFromMean(temp_meancurvedata, temp_meancurvesum, ...
        temp_meancurvecount, temp_meancurvemean, temp_meancurvestd, best_pdoffset, amNormcube, amInterventions, intervention, ...
...
for offset = 0:max_offset-1
    ...
    rowtoadd1.CalcValue = 0;
    rowtoadd1(:,nondaycols1+1:nondaycols1+align_wind) = array2table(reshape(temp_meancurvemean(max_offset - offset:max_offset+align_wind - 1 - offset, measure), [1 align_wind]));
    datawindowtable = [datawindowtable ; rowtoadd1];
    ...
    rowtoadd1.CalcValue = exp(-1 * (best_histogram(measure, intervention, offset+1) - min(best_histogram(measure, intervention, offset+1))));
    rowtoadd1(:,nondaycols1+1:nondaycols1+align_wind) = array2table([0]);
    datawindowtable = [datawindowtable ; rowtoadd1];
    
    rowtoadd1.RowType = 'PosteriorProb';
    rowtoadd1.CalcValue = best_pdoffset(measure, intervention, offset+1);
    rowtoadd1(:,nondaycols1+1:nondaycols1+align_wind) = array2table([0]);
    datawindowtable = [datawindowtable ; rowtoadd1];
