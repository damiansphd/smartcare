...
colname1 = sprintf('%s%sPre%sDurby%s', meastype, twtext, twtext, bwtext);
...
for n = 1:nperiods
    labeltext{n + 2} = sprintf('>=%d<%dPre', twindow - ((n-1) * bestwind), (twindow - (n * bestwind)));
end
for n = 1:nperiods
    labeltext{nperiods + n + 2} = sprintf('>=%d<%dDur', (n-1) * bestwind, n * bestwind);
end
labeltext{(nperiods * 2) + 3} = sprintf('>=%dDur', twindow);
